...
class LoginScreen extends Component {
  ...
  async componentDidMount() {
    await this.setState({
      filteredMovie: this.state.dataMovie,
      loading: false
    });
  ...
  filterMovies = (textMovie) => {
    let moviesCopy = this.state.dataMovie;
     let results = moviesCopy.filter(movie => movie.title.indexOf(textMovie) === 0);
  ...
  _renderItem = ({ item }) => (
    <View style={styles.SingleItem}>
      <TouchableOpacity onPress={() => NavigationService.navigate("DetailMovie", {
        ...
        <Image style={{ width: '100%', height: 200 }} source={{ uri: `https://image.tmdb.org/t/p/w342${item.poster_path}` }} />
